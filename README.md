...
| 🎧 **视听练习** | 七种题型：指板认音、音阶填空、听音辨名、音程听辨、音阶听辨，以及**五线谱视奏**与**简谱视奏** —— 同一个「读谱后在指板上依次点出」，一张画带调号与节奏的五线谱（谱表跟定弦自动走），一张画首调数字简谱（六条体系实例 × 各 7 条子音阶 = 42 条） |
...
- **读谱辅助**：**▶ 试听题目**按谱面音高依次放一遍（休止符只占时间不出声）；
  **☐ 显示音名**（默认关）把每个音的音名标在谱下。
- 作答行实时记录你点过的音（带序号），支持 **↶ 撤回上一步** 与**清空作答**，点满后按 **✅ 提交答案**。
- **每题音符数自己定**（3–12，默认 5），按 8 组母音阶分组出题；题型选择、音符数与音名开关都存进本机偏好。

## 看数字谱在指板上找音：简谱视奏

视听练习里的**第 7 个题型**，是上一节那件事的另一种读法。题目画成**简谱** ——
首调数字、高低音点、升降号、时值记号（八分下划线 / 附点 / 二分与全音符的增时线）、
小节线一应俱全；节奏与五线谱视奏**完全同一套**出题槽位。读谱之后，在指板上按顺序点出对应的音。

- **题板左上角直接写出 `1 = X`** —— 简谱的灵魂是「1 落在哪」，不写清就没法读。
  三支小调体系里两支按「关系大调主音为 1」写（所以是 `6712345` 起头）。
  **旋律小调两套都留、在面板上占两行**：`12b34567`（1 = la，以自身主音为 1，
  一眼看出「上行还原了 6 / 7 级」）与 `67123#4#5`（1 = do，以关系大调主音为 1，
  才看得出「大调降三级」）—— 两套都留着练，勾选互不影响。
- **六条体系实例 × 各 7 条子音阶 = 42 条**，子音阶一律**按首调记**：
  母音阶基串**循环左移**就得到下一级调式的写法。自然大调 `1234567` → 多利亚 `2345671`
  → 弗里吉亚 `3456712` → 利底亚 `4567123` → 混合利底亚 `5671234` → 自然小调 `6712345`
  → 洛克里亚 `7123456`；和声小调 `671234#5`、和声大调 `12345b67`、双和声小调 `671#234#5`
  同理；旋律小调两套记谱法各 7 条（1 = la 的 `12b34567`… 与 1 = do 的 `67123#4#5`…）。
- **两级勾选**：体系那一级只管「全选 / 全不选」，真正的出题库是**子音阶那一级**
  （某体系只勾了一部分时，那一行显示为**半选**）。全部取消会自动回落成六条体系实例全选（42 条）。
- 每题音符数同样 3–12（默认 5）。
- 其余一律与五线谱视奏相同：**点一下同音高的位置全亮**、**判定卡八度**、
  **▶ 试听题目 / ☐ 显示音名**（默认关，要看得自己打开；这里标的是**实发音名**，简谱没有谱表八度这回事）、
  **↶ 撤回上一步 / ✅ 提交答案**，全对自动进下一题。
...
- **位置是一块一块选的**：一块 = 通用把位分支的一个把位 / CAGED 的一份落位 / 三品窗的一段。
  全指板下五声 / 布鲁斯族 **81** 处、六声族 **94** 处、七 / 八声族各 **107** 处。
  **CAGED 栏按型分行**（C 型 / A 型 / G 型 / E 型 / D 型），行内**按品从低到高**列出该型的
  每一个具体把位（标准定弦下五型共 **10** 处）—— 同一型的高低把位可以分别点到。
  选中的进一个可拖动的**练习顺序槽**；段序两档可切（默认按品序，也可按你点选的次序）。
  **换把区在指板图上自动标注**。
