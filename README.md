...
| 🎯 **和弦查询** | 反过来：在指板上点音（最少 3 个）**反推和弦**。粗匹配 / 音级匹配 / 调内和弦三种口径；低音不是根音就写斜线和弦；对不上任何和弦时按「去掉根音拆成两个三音组」的五族体系兜底分类 |
...
- **单文件**：整个应用（HTML + CSS + JS）就是 `index.html` 一个文件，**11,388 行 / 769 KB**。无构建步骤、无依赖、无 CDN。
